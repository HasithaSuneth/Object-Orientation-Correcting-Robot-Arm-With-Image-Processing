...
def orientation():
	...
	lower=[(0, 100, 100)] #0,130,100
	upper=[(100, 255, 255)] #70,255,255

	img1 = cv.imread('images_temp/normal_crop_img.jpg')
	img2 = cv.imread('images_temp/saved_img.jpg')	

	edged = cv.Canny(img2, 0, 200) #150 550 # 0 200
	pts = np.argwhere(edged>0)
	y1, x1 = pts.min(axis=0)
	y2, x2 = pts.max(axis=0)
	croppedimg2 = img2[y1:y2, x1:x2]
	cv.imwrite('images_temp/croped_img2.jpg', croppedimg2)
	...
	#output1 = cv.bitwise_and(img1, img1, mask = mask1)
	#output2 = cv.bitwise_and(img2, img2, mask = mask2)

	img_shape1 = img1.shape
	img_shape2 = croppedimg2.shape
	#print("1 ", img_shape1)
	#print("2 ", img_shape2)
	if img_shape1[0]> img_shape2[0] or img_shape1[0] == img_shape2[0]:
	...
	for b in range(len(myList)):
		...
		#print(b," black pix", blackPixList[b])
